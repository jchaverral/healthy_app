...
def predict_age(model):
    ...
    input_data = pd.DataFrame({
        ...
        'Smoking Status': [2 if Smoking_Status == "No fuma" else 1 if Smoking_Status == "Fuma poco" else 2 ],
        ...
        'Sleep Patterns': [1 if Sleep_Patterns == "Insomnio" else 2 if Physical_Activity_Level == "Normal" else 0],
        'Stress Levels': [Stress_Levels],
        'Pollution Exposure': [Pollution_Exposure],
        'Sun Exposure': [Sun_Exposure],
        'Income Level': [1 if Income_Level == "Bajo" else 21 if Income_Level == "Medio" else 0]        
         })
